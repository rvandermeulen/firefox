...
import org.mozilla.fenix.benchmark.utils.TRACE_NAME_ANIMATION_TAB_MANAGER_TO_THUMBNAIL
import org.mozilla.fenix.benchmark.utils.TRACE_NAME_ANIMATION_THUMBNAIL_TO_TAB_MANAGER
...
class TabsTrayBenchmark {
    ...
    @RequiresApi(Build.VERSION_CODES.Q)
    @Test
    fun switchTabsAnimationOff() = switchTabsBenchmarkAnimationOff(CompilationMode.Partial(baselineProfileMode = BaselineProfileMode.Require))

    @OptIn(ExperimentalMetricApi::class)
    @RequiresApi(Build.VERSION_CODES.Q)
    private fun switchTabsBenchmarkAnimationOn(compilationMode: CompilationMode) {
        var firstStart = true
        ...
        benchmarkRule.measureRepeatedDefault(
            ...
            metrics = listOf(
                StartupTimingMetric(),
                FrameTimingMetric(),
                TraceSectionMetric(sectionName = TRACE_NAME_ANIMATION_TAB_MANAGER_TO_THUMBNAIL, mode = TraceSectionMetric.Mode.Average),
                TraceSectionMetric(sectionName = TRACE_NAME_ANIMATION_THUMBNAIL_TO_TAB_MANAGER, mode = TraceSectionMetric.Mode.Average)
            ),
    ...
    @OptIn(ExperimentalMetricApi::class)
    @RequiresApi(Build.VERSION_CODES.Q)
    private fun switchTabsBenchmarkAnimationOff(compilationMode: CompilationMode) {
        var firstStart = true
        val useComposableToolbar = true
